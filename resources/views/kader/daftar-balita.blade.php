...
@php
    ...
    $percentage = $totalAnak > 0 ? round(((int) ($statSelesai ?? 0) / $totalAnak) * 100) : 0;
@endphp
...
<div class="w-full mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 pb-6 flex flex-col gap-5 lg:gap-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3">
            <span class="inline-flex items-center gap-2 text-sm text-slate-600 font-medium">
                <x-icon name="chart-pie-slice" weight="bold" class="text-teal-600 text-base" />
                Progress <span class="font-bold text-slate-900 tabular-nums">{{ $percentage }}%</span>
                <span class="text-slate-400">({{ $statSelesai ?? 0 }}/{{ $totalAnak }} terukur)</span>
            </span>
        </div>
    </div>
