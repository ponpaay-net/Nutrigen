    ...
    <div x-show="tab === 'info'" x-cloak class="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-6 items-stretch">
        ...
        <section class="bg-white border border-slate-200 rounded-2xl shadow-[0_1px_3px_rgba(15,23,42,0.06)] overflow-hidden flex flex-col">
            ...
            <div class="p-5 flex flex-col gap-0.5 flex-1">
                <div class="flex items-center gap-2.5 mb-2"><span class="w-9 h-9 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center"><x-icon name="identification-card" weight="bold" class="text-[17px]" /></span><h4 class="text-[14px] font-bold text-slate-900">Identitas Balita</h4></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Nama</span><span class="text-[14px] font-semibold text-teal-700 text-right">{{ $childName ?: '—' }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">NIK</span><span class="text-[14px] font-semibold text-slate-800 font-mono tabular-nums text-right break-all">{{ $nik ?: '—' }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Tgl Lahir</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ $birthDate }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Kelamin</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ $gender }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">No BPJS</span><span class="text-[14px] font-semibold text-slate-800 font-mono tabular-nums text-right">{{ $noBpjs ?: '—' }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Usia</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ $age }}</span></div>
            </div>
        </section>
        ...
        <section class="bg-white border border-slate-200 rounded-2xl shadow-[0_1px_3px_rgba(15,23,42,0.06)] overflow-hidden flex flex-col">
            ...
            <div class="p-5 flex flex-col gap-0.5 flex-1">
                <div class="flex items-center gap-2.5 mb-2"><span class="w-9 h-9 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center"><x-icon name="users" weight="bold" class="text-[17px]" /></span><h4 class="text-[14px] font-bold text-slate-900">Orang Tua & Domisili</h4></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Ibu</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ $motherName ?: '—' }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Ayah</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ $fatherName ?: '—' }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Kontak Ibu</span><span class="text-[14px] font-semibold text-slate-800 font-mono tabular-nums text-right">{{ $motherPhone ?: '—' }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5 border-b border-slate-100"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Domisili</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ ($address ?: '—') . ($addressSub ? ', ' . $addressSub : '') }}</span></div>
                <div class="flex items-center justify-between gap-3 py-2.5"><span class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 shrink-0">Posyandu</span><span class="text-[14px] font-semibold text-slate-800 text-right">{{ $posyanduName ?: '—' }}</span></div>
            </div>
        </section>
    </div>
                        ...
                        <template x-if="active !== null">
                            <div>
                                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                                    <div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full" x-bind:class="items[active]?.status_validasi === 'rejected' ? 'bg-rose-500' : (items[active]?.status_validasi === 'pending' ? 'bg-amber-400' : 'bg-emerald-500')"></span><span class="text-[15px] font-bold text-slate-900" x-text="items[active]?.date"></span><span class="text-[12px] text-slate-400" x-text="'· ' + (items[active]?.age_at_measure || '')"></span></div>
                                ...
                                <div class="p-5 flex flex-col gap-4 overflow-y-auto">
                                    ...
                                    <div class="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-lg"><span class="text-[12px] font-semibold text-slate-500">Status</span><span class="text-[13px] font-semibold" x-bind:class="items[active]?.status_validasi === 'rejected' ? 'text-rose-600' : (items[active]?.status_validasi === 'pending' ? 'text-amber-600' : 'text-emerald-600')" x-text="items[active]?.status"></span></div>
                                    <template x-if="items[active]?.status_validasi === 'rejected' && items[active]?.catatan_validator">
                                        <div class="border border-rose-200 rounded-xl p-3.5 flex items-start gap-2.5 bg-rose-50/40"><x-icon name="chat-circle-text" weight="fill" class="text-rose-500 text-[19px] shrink-0 mt-0.5" /><div class="min-w-0"><p class="text-[11px] font-bold text-rose-600 uppercase tracking-wide">Catatan Petugas Gizi Puskesmas</p><p class="text-[13px] text-slate-700 mt-1 leading-relaxed" x-text="items[active]?.catatan_validator"></p></div></div>
                                    </template>
                                </div>
