...
@php
    $inp = 'w-full h-12 rounded-xl border border-slate-200 bg-slate-50 px-4 text-[15px] font-semibold text-slate-800 placeholder:text-slate-300 shadow-sm focus:outline-none focus:ring-4 focus:ring-teal-500/15 focus:border-teal-600 focus:bg-white transition-all';
...
<div class="bg-slate-50 min-h-[100dvh]">
    <div class="max-w-2xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight leading-tight">Ukur Balita</h1>
                <p class="text-[13px] text-slate-500">Catat pertumbuhan saat hari posyandu.</p>
            </div>
        </div>

        {{-- Card 1: Ringkasan anak --}}
        <div class="bg-white rounded-2xl border border-slate-200 shadow-[0_1px_3px_rgba(15,23,42,0.06),0_16px_40px_-24px_rgba(15,23,42,0.18)] overflow-hidden mb-6">
            <div class="px-5 sm:px-7 py-4 bg-gradient-to-r from-teal-600 to-teal-700 text-white">
                <div class="flex items-center gap-3.5">
                    <div class="w-12 h-12 shrink-0 rounded-xl bg-white/20 ring-1 ring-white/30 flex items-center justify-center">
                        <span class="text-lg font-black">{{ strtoupper(substr($childName, 0, 1)) }}</span>
                    </div>
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            <h2 class="text-[16px] font-bold leading-tight truncate">{{ $childName }}</h2>
                            <span class="text-[11px] font-bold bg-white/20 px-2 py-0.5 rounded-lg">{{ $age }}</span>
                        </div>
                        <p class="text-[12.5px] text-teal-50 mt-0.5">{{ $gender === 'L' ? 'Laki-laki' : 'Perempuan' }} · Lahir {{ $birthDate }}</p>
                    </div>
                </div>
            </div>
            @if($lastDate)
            <div class="px-5 sm:px-7 py-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px]">
                <span class="inline-flex items-center gap-1.5 text-slate-500 font-medium"><x-icon name="clock" weight="bold" class="text-[14px] text-teal-600" /> Pengukuran terakhir</span>
                ...
                @if($lastStatus)<span class="text-[11.5px] font-semibold text-teal-700 bg-teal-50 border border-teal-200 px-2 py-0.5 rounded-lg">{{ is_array($lastStatus) ? ($lastStatus['label'] ?? 'Normal') : $lastStatus }}</span>@endif
        ...
        @if($errors->any())
        <div class="mb-6 rounded-xl bg-rose-50 border border-rose-200 px-4 py-3 text-[13px] text-rose-700">
        ...
        <form id="measurementForm" action="{{ route('pengukuran.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-[0_1px_3px_rgba(15,23,42,0.06),0_16px_40px_-24px_rgba(15,23,42,0.18)] overflow-hidden">
            ...
            <div class="p-5 sm:p-7 space-y-8">
                ...
                <section>
                    <div class="flex items-center gap-3 mb-5 pb-4 border-b border-slate-100">
                        ...
                        <div><h2 class="text-[15px] font-bold text-slate-900 leading-tight">Antropometri</h2><p class="text-[12.5px] text-slate-500">Ukuran fisik balita saat ini.</p></div>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <div class="{{ $field }}">
                            ...
                            <div class="relative">
                                ...
                                <span class="absolute right-4 inset-y-0 flex items-center text-[13px] font-bold text-slate-400">kg</span>
                            </div>
                        ...
                        <div class="{{ $field }}">
                            ...
                            <div class="relative">
                                ...
                                <span class="absolute right-4 inset-y-0 flex items-center text-[13px] font-bold text-slate-400">cm</span>
                            </div>
                ...
                <section>
                    <div class="flex items-center gap-3 mb-5 pb-4 border-b border-slate-100">
                        ...
                        <div><h2 class="text-[15px] font-bold text-slate-900 leading-tight">Status & Informasi</h2><p class="text-[12.5px] text-slate-500">Kenaikan berat, ASI, tanggal & catatan.</p></div>
                    </div>
            ...
            {{-- Sticky submit --}}
            <div class="sticky bottom-0 bg-white/95 backdrop-blur border-t border-slate-100 px-5 sm:px-7 py-4 flex items-center justify-end gap-3">
                <a href="{{ route('balita.show', $balitaId) }}" class="h-12 px-5 rounded-xl border border-slate-200 bg-white text-slate-700 text-[14px] font-semibold hover:bg-slate-50 transition-colors inline-flex items-center justify-center">Batal</a>
                <button type="submit" id="btn-submit"
                    class="flex-1 sm:flex-initial h-12 px-7 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-[14px] font-bold transition-colors inline-flex items-center justify-center gap-2 shadow-md shadow-teal-600/20">
