        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-6" x-data="{ showCurrent:false, showNew:false, showConfirm:false }">
                    ...
                    <div class="p-6 sm:p-8">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" @click="showCurrent = !showCurrent" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-teal-600 transition-colors" :aria-label="showCurrent ? 'Sembunyikan' : 'Lihat'">
                        ...
                        <div class="flex flex-col gap-2 mt-5">
                            ...
                            <div class="relative">
                                <input :type="showNew ? 'text' : 'password'" id="password" name="password" required minlength="8" placeholder="Minimal 8 karakter"
                                    class="w-full bg-slate-50 border border-slate-200 hover:border-teal-300 text-slate-900 text-[14px] font-medium rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 transition-all placeholder:text-slate-400">
                                <button type="button" @click="showNew = !showNew" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-teal-600 transition-colors"><x-icon name="eye" weight="bold" class="text-[18px]" x-show="!showNew" /><x-icon name="eye-slash" weight="bold" class="text-[18px]" x-show="showNew" x-cloak /></button>
                            </div>
                            <p class="text-[11.5px] text-slate-400 font-medium mt-1.5">Minimal 8 karakter, gunakan kombinasi huruf besar, kecil, angka, dan simbol.</p>
            ...
            <div class="flex flex-col gap-5 lg:sticky lg:top-24">
                ...
                <div class="bg-white ring-1 ring-slate-200/70 rounded-2xl shadow-sm shadow-slate-200/50 p-5 sm:p-6">
                    ...
                    <p class="text-[11px] text-slate-400 mt-2">Terhubung ke {{ Auth::user()->email ?? '-' }}</p>
                </div>
            </div>
        </div>
